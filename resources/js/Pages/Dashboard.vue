    ...
    <AuthenticatedLayout>
        ...
        <div v-if="summaryOpen && flash?.daySummary" class="fixed inset-0 z-[80] grid place-items-center overflow-y-auto bg-[#071729]/60 p-4" @click.self="summaryOpen = false">
            <div class="w-full max-w-2xl rounded-md bg-white shadow-2xl">
                ...
                <div class="p-6"><div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div class="rounded-md bg-[#f4f8fa] p-4"><div class="text-xs text-[#718599]">Faturamento</div><div class="mt-1 text-xl font-bold text-[#19324d]">{{ formatMoney(flash.daySummary.sales_revenue_cents) }}</div></div><div class="rounded-md bg-[#f4f8fa] p-4"><div class="text-xs text-[#718599]">Unidades vendidas</div><div class="mt-1 text-xl font-bold text-[#19324d]">{{ flash.daySummary.units_sold }} / {{ flash.daySummary.commercial_capacity_units }}</div><div class="text-[10px] text-[#8393a3]">vendidas / capacidade</div></div><div class="rounded-md bg-[#f4f8fa] p-4"><div class="text-xs text-[#718599]">Lucro bruto</div><div class="mt-1 text-xl font-bold text-[#19324d]">{{ formatMoney(flash.daySummary.gross_profit_cents) }}</div></div><div class="rounded-md bg-[#f4f8fa] p-4"><div class="text-xs text-[#718599]">Variação do caixa</div><div class="mt-1 text-xl font-bold" :class="flash.daySummary.cash_change_cents >= 0 ? 'text-[#087c68]' : 'text-[#d65737]'">{{ formatMoney(flash.daySummary.cash_change_cents) }}</div></div></div><div class="mt-5 grid gap-3 sm:grid-cols-4"><div class="rounded-md border border-[#dfe7ee] p-4 text-sm text-[#5f7488]"><strong class="block text-[#263e56]">{{ formatMoney(flash.daySummary.expenses_cents) }}</strong>Obrigações pagas</div><div class="rounded-md border border-[#dfe7ee] p-4 text-sm text-[#5f7488]"><strong class="block text-[#263e56]">{{ flash.daySummary.received_purchase_order_ids.length }}</strong>Entregas recebidas</div><div class="rounded-md border border-[#dfe7ee] p-4 text-sm text-[#5f7488]"><strong class="block text-[#263e56]">{{ flash.daySummary.stockout_product_ids.length }}</strong>Produtos com ruptura</div><div class="rounded-md border border-[#dfe7ee] p-4 text-sm text-[#5f7488]"><strong class="block text-[#263e56]">{{ flash.daySummary.unmet_demand_units }}</strong>Demanda não atendida</div></div><div v-if="flash.daySummary.event" class="mt-5 flex items-start gap-3 rounded-md border border-[#f3c9a7] bg-[#fff7ee] p-4"><Flame :size="20" class="mt-0.5 shrink-0 text-[#d65c2f]" /><div><div class="text-xs font-bold uppercase text-[#c45027]">Evento do dia</div><div class="mt-0.5 text-sm font-bold text-[#19324d]">{{ flash.daySummary.event.title }}</div><p class="mt-1 text-xs text-[#6b7f93]">{{ flash.daySummary.event.description }}</p></div></div><div v-if="flash.daySummary.game_status !== 'active'" class="mt-5 rounded-md p-4 text-center" :class="flash.daySummary.game_status === 'won' ? 'bg-[#e7f8f3] text-[#087c68]' : 'bg-[#fff0ed] text-[#c44032]' "><strong class="block">{{ flash.daySummary.game_status === 'won' ? 'Vitória alcançada' : 'Partida encerrada por falência' }}</strong><span class="text-xs">Patrimônio final: {{ formatMoney(flash.daySummary.equity_cents) }}</span></div><button type="button" class="mt-6 h-11 w-full rounded-md bg-[#173f67] text-sm font-bold text-white" @click="summaryOpen = false">Continuar</button></div>
            </div>
        </div>
