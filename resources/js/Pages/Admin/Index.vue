...
import { BriefcaseBusiness, ContactRound, Gamepad2, LayoutDashboard, Package, Pencil, Plus, Save, Search, ShieldCheck, Trash2, Users, X } from '@lucide/vue';
...
const props = defineProps<{
    section: 'overview' | 'users' | 'games' | 'population' | 'products' | 'job-roles';
    ...
    records?: Paginated<UserRecord | GameRecord | PopulationRecord | ProductRecord | JobRoleRecord>;
            ...
            <template v-else>
                <div class="mt-6 grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
                    <section class="self-start border border-[#dfe7ee] bg-white p-5 shadow-sm">
                        <div class="mb-5 flex items-center justify-between"><h2 class="text-sm font-bold text-[#19324d]">{{ editingId ? 'Editar registro' : section === 'games' ? 'Selecione uma partida' : 'Novo registro' }}</h2><button v-if="editingId" type="button" class="grid size-8 place-items-center text-[#718499]" title="Cancelar edição" @click="resetEditor"><X :size="18" /></button></div>
                    ...
                    <section class="min-w-0 overflow-hidden border border-[#dfe7ee] bg-white shadow-sm">
                        <div class="flex flex-col gap-3 border-b border-[#e6edf2] p-4 sm:flex-row sm:items-center sm:justify-between"><form class="flex max-w-2xl flex-1 flex-col gap-2 sm:flex-row" @submit.prevent="runSearch"><div class="relative flex-1"><Search :size="17" class="absolute left-3 top-2.5 text-[#8293a4]" /><input v-model="search" placeholder="Buscar..." class="h-10 w-full rounded-md border-[#ccd8e1] pl-9 text-sm" /></div><button class="h-10 rounded-md bg-[#173f67] px-4 text-sm font-bold text-white">Buscar</button></form><button v-if="section !== 'games'" type="button" class="flex h-10 items-center justify-center gap-2 rounded-md border border-[#ccd8e1] px-3 text-sm font-semibold text-[#526a80] xl:hidden" @click="resetEditor"><Plus :size="16" /> Novo</button></div>
