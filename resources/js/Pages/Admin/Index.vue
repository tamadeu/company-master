...
interface JobRoleRecord { id: number; department: string; name: string; salary_cents: number; sales_capacity_units: number; inventory_capacity_units: number; purchasing_capacity_units: number; active: boolean }
...
const roleForm = useForm({ department: '', name: '', salary_cents: 0, sales_capacity_units: 0, inventory_capacity_units: 0, purchasing_capacity_units: 0, active: true });
...
const editJobRole = (jobRole: JobRoleRecord) => {
    ...
    roleForm.purchasing_capacity_units = jobRole.purchasing_capacity_units;
                ...
                <div class="mt-6 grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
                    ...
                    <section class="min-w-0 overflow-hidden border border-[#dfe7ee] bg-white shadow-sm">
                        ...
                        <div class="overflow-x-auto">
                            ...
                            <table v-if="section === 'job-roles'" class="w-full min-w-[950px] text-left"><thead class="bg-[#f8fafc] text-xs uppercase text-[#74879a]"><tr><th class="px-5 py-3">Cargo</th><th class="px-4 py-3 text-right">Salário</th><th class="px-4 py-3 text-right">Cap. vendas</th><th class="px-4 py-3 text-right">Cap. estoque</th><th class="px-4 py-3 text-right">Cap. compras</th><th class="px-4 py-3">Status</th><th class="px-5 py-3 text-right">Ações</th></tr></thead><tbody class="divide-y divide-[#edf1f4]"><tr v-for="jobRole in jobRoles?.data" :key="jobRole.id"><td class="px-5 py-4"><div class="text-sm font-semibold">{{ jobRole.name }}</div><div class="text-xs text-[#718499]">{{ jobRole.department }}</div></td><td class="px-4 py-4 text-right text-sm">{{ formatMoney(jobRole.salary_cents) }}</td><td class="px-4 py-4 text-right text-sm">{{ jobRole.sales_capacity_units }}</td><td class="px-4 py-4 text-right text-sm">{{ jobRole.inventory_capacity_units }}</td><td class="px-4 py-4 text-right text-sm font-semibold text-[#17638d]">{{ jobRole.purchasing_capacity_units }}</td><td class="px-4 py-4 text-xs font-bold" :class="jobRole.active ? 'text-[#087c68]' : 'text-[#9a5b42]'">{{ jobRole.active ? 'Ativo' : 'Inativo' }}</td><td class="px-5 py-4"><div class="flex justify-end gap-1"><button title="Editar" class="grid size-9 place-items-center text-[#17638d]" @click="editJobRole(jobRole)"><Pencil :size="16" /></button><button title="Excluir" class="grid size-9 place-items-center text-red-600" @click="destroyRecord('admin.job-roles.destroy', jobRole.id, `Excluir o cargo ${jobRole.name}?`)"><Trash2 :size="16" /></button></div></td></tr></tbody></table>
                        </div>
