...
<template>
    ...
    <AuthenticatedLayout>
        <div class="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            <section class="mt-4 rounded-md border border-[#dfe7ee] bg-white p-5 shadow-sm"><div class="flex items-start gap-3"><Users :size="21" class="mt-0.5 shrink-0 text-[#27658e]" /><div><h2 class="text-sm font-bold text-[#19324d]">Regra operacional</h2><p class="mt-1 text-sm leading-6 text-[#6b7f93]">As vendas são processadas em background ao longo do dia e respeitam demanda, estoque e capacidade comercial acumulada. Configuração de preço fica em <strong>Estoque e produtos</strong>. O gestor cobre {{ salesRules.ownerBaseCapacityUnits }} un./dia e a equipe Comercial amplia o limite.</p><p v-if="salesRules.latestUnmetDemandUnits" class="mt-1 text-xs font-semibold text-[#c45027]">Último dia: {{ salesRules.latestUnmetDemandUnits }} unidade(s) de demanda não atendida.</p></div></div></section>
        </div>
    </AuthenticatedLayout>
</template>